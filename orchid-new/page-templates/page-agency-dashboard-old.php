<?php
    //Global wordpress database connection
    global $wpdb;
    $conn = new wpdb('root','Mangopen#61!','wordpress2','localhost');
    //create the DB connection
    $pl = "PL";
    $cl = "CL";
    //Get the tiles from the wp_tiles table
    $resultspA = "SELECT * FROM wp_tiles WHERE type='PL' ORDER BY tile_order ASC";
    $resultscB = "SELECT * FROM wp_tiles WHERE type='CL' ORDER BY tile_order ASC";

    $resultsp = $conn->get_results($resultspA);
    $resultsc = $conn->get_results($resultscB);
 ?>
 <?php //$wpdb->show_errors(); ?>
 <?php //$wpdb->print_error(); ?>
<?php
/*
Template Name: AgencyDashboard */
?>

<!-- Start Page main header -->
<?php get_header(); ?>


 <!---<section class="header-pages-ag" style="height:80px;">
   <div class="container normal">
       <div class="text-holder">

       </div>
   </div>
 </section>--->

  <section class="main-content">
      <script>
        window.onload = showTour;
      </script>
    <div class="container-fluid" style="width:85.8%;">
        <h1 class="h1-a">Agency Dashboard</h1>
    <div id="search">

    </div>
    <div class="row">
       <div class="table-responsive">
            <table width="85%" style="border:0px;border-width: 0 0 0 0;cellpadding:0px;padding:0px;">
              <tr>
                <td style="border:0px;border-width: 0 0 0 0;"><span style="color:#0057B8;font-weight:bold;font-size:1.2em;">New Business: </span></td>
                <td style="border:0px;border-width: 0 0 0 0;"><span style="color:#0057B8;font-weight:bold;font-size:1.2em;">Existing Policies and Quotes:</span></td>
                <td width="13.8%" rowspan="3" style="border:0px;border-width: 0 0 0 0;text-align:right;padding:0;" align="right">
                    <div id="contactorchid" style="float:left;border:1px solid #ccc;width:95%;height:auto;padding:0;box-shadow: 3px 3px #cccccc;">
 			<div style="float:left;margin-left:10px;">
    			  <h5 class="h5-a">Contact Orchid</h5>
   			  <p class="agenncy"><strong><i class="fa fa-phone-square fa-lg" aria-hidden="true"></i> :</strong> 772-226-5546</p>
			</div>
  		    </div>
                </td>
              </tr>
              <tr>
                <td id="newbiztour" style="border:0px;border-width: 0 0 0 0;">
                  <p>
                    <select id="fstate" name="fstate" style="float:left;padding:8px;width:250px;color:#5a5a5a; border:1px solid #cccccc;background-color:#fff;" onchange="getCaribb();">
                  <option value="all" selected='true'>Select a property location</option>
                  <option value="Caribbean" style="background:#f0f4c3;">Caribbean Properties</option>
                  <option value="Alabama" data-postid="721">Alabama</option>
                  <option value="Arizona" data-postid="1599">Arizona</option>
                  <option value="California" data-postid="1601">California</option>
                  <option value="Colorado" data-postid="1603">Colorado</option>
                  <option value="Connecticut" data-postid="1605">Connecticut</option>
                  <option value="Delaware" data-postid="1686">Delaware</option>
                  <option value="District of Columbia" data-postid="0">District of Columbia</option>
                  <option value="Florida" data-postid="691">Florida</option>
                  <option value="Georgia" data-postid="1608">Georgia</option>
                  <option value="Hawaii" data-postid="1611">Hawaii</option>
                  <option value="Idaho" data-postid="1614">Idaho</option>
                  <option value="Illinois" data-postid="1616">Illinios</option>
                  <option value="Indiana" data-postid="1618">Indiana</option>
                  <option value="Kansas" data-postid="1621">Kansas</option>
                  <option value="Kentucky" data-postid="1624">Kentucky</option>
                  <option value="Louisiana" data-postid="1626">Louisiana</option>
                  <option value="Maine" data-postid="1628">Maine</option>
                  <option value="Maryland" data-postid="1630">Maryland</option>
                  <option value="Massachusetts" data-postid="1249">Massachusetts</option>
                  <option value="Michigan" data-postid="1632">Michigan</option>
                  <option value="Minnesota" data-postid="1635">Minnesota</option>
                  <option value="Mississippi" data-postid="1637">Mississippi</option>
                  <option value="Missouri" data-postid="1640">Missouri</option>
                  <option value="Montana" data-postid="1642">Montana</option>
                  <option value="Nevada" data-postid="1644">Nevada</option>
                  <option value="New Hampshire" data-postid="1649">New Hampshire</option>
                  <option value="New Jersey" data-postid="1646">New Jersey</option>
                  <option value="New York" data-postid="1275">New York</option>
                  <option value="North Carolina" data-postid="1679">North Carolina</option>
                  <option value="Ohio" data-postid="1651">Ohio</option>
                  <option value="Oklahoma" data-postid="1653">Oklahoma</option>
                  <option value="Oregon" data-postid="1655">Oregon</option>
                  <option value="Pennsylvania" data-postid="1657">Pennsylvania</option>
                  <option value="Rhode Island" data-postid="1659">Rhode Island</option>
                  <option value="South Carolina" data-postid="1681">South Carolina</option>
                  <option value="Tennessee" data-postid="1661">Tennessee</option>
                  <option value="Texas" data-postid="1683">Texas</option>
                  <option value="Utah" data-postid="1663">Utah</option>
                  <option value="Vermont" data-postid="1665">Vermont</option>
                  <option value="Virginia" data-postid="1667">Virginia</option>
                  <option value="Washington" data-postid="1670">Washington</option>
                  <option value="Wisconsin" data-postid="1675">Wisconsin</option>
                  <option value="West Virginia" data-postid="1672">West Virginia</option>
                  <option value="Wyoming" data-postid="1677">Wyoming</option>
                </select>
              </p>
              <br/>
              </td>
                <td id="exbiztour" valign="top" style="border:0px;border-width: 0 0 0 0;">
                  <button class="btn btn-info" title="Orchid Connect" onclick="window.open('http://connect.orchidinsurance.com','_blank');"><i class="orc-orchid-logo" aria-hidden="true" style="color:#fff;font-size:18px;"></i>Orchid Connect</button> <button class="btn btn-info" title="Coastal Agents Alliance" onclick="defaultLogin('1');"><i class="coast-coastal-logo" aria-hidden="true" style="color:#fff;font-size:18px;"></i>Coastal Agents Alliance</button> <button class="btn btn-info" title="Orchid PolicyPlus" onclick="defaultLogin('3');"><i class="orc-orchid-logo" aria-hidden="true" style="color:#fff;font-size:18px;"></i>Orchid Policy Plus</button>
                  <div style="float:right;margin:5px 30px 0px 0px;font-weight:500;"><a href="http://orchidinsurance.com/agency-dashboard-instructions/" style="font-weight:500;" target="_blank" title="Introduction Video">Introduction Video</a></div>
                </td>

              </tr>
              <tr>
                <td colspan="2" align="left" style="border:0px;border-width: 0 0 0 0;">
                  <div style="display:inline;float:left;">
                <label style="position:relative;top:0px;font-size:0.8em;font-weight:normal;">Make this my default location:<label> <input type="checkbox" value="1" id="locdefault" onclick="setCookies('OrchidDash',fstate,'365');" name="locdefault" style="position:relative;top:5px;" />
                  </div>
                </td>
              </tr>
              <tr><td colspan="2" style="border:0px;border-width: 0 0 0 0;">
                  </td>
              </tr>
            </table>
          </div>
    </div>
    <!-- START Content -->
    <div id="content">
         <div id="left-cont">
             <?php if ( have_posts() ) : while ( have_posts() ) : the_post(); ?>
                 <div class="col-lg-4 col-md-4 ">
                     <div class="lead text-center" style="color:#0057b8">Personal Lines<br/>
                         <span style="font-size:0.8em;">Select a product to create a new quote.</span>
                     </div>
                     <div class="container-fluid center">
                       <div class="rowP" style="padding-left:30px;padding-top:8px;">
                 <?php foreach ($resultsp as $rowp) { //Start Personal Lines ?>
                            <div class="filterrowP" data-state="<?php echo $rowp->states; ?>">
                                <div class="col-sm-4" style="background-color:#FFFFFF;">
                                    <?php if ($rowp->occurring == '2') { //Check for coming soon YES=2?>
                                        <span class="sm-4-corner">Coming soon</span>
                                        <div  name="footprintinfo" class="footprintinfo" style="position:absolute;left:122px;z-index:2147483647;"></div>
                                        <img src="wp-content/themes/orchid-new/assets/img/<?php echo $rowp->image ?>" border="0" class="ind-a" title="<?php echo $rowp->tile_name ?>" />
                                        <div class="tiles-t"><?php echo $rowp->tile_name ?></div>
                                    <?php } else if ($rowp->occurring == '1') { //Check for coming soon NO=1 ?>
                                        <div  name="footprintinfo" class="footprintinfo" style="position:absolute;left:122px;z-index:2147483647;"></div>
                                            <?php if ($rowp->app_url == 'none' && $rowp->app_type == '3') {  //Check to see if there is a PDF or Link ?>
                                                <a href="#" onclick='getLine("<?php echo $rowp->getLine ?>","<?php echo $rowp->tile_name ?>","none");' class="href-nav" title="<?php echo $rowp->tile_name ?>">
                                            <?php } else if ($rowp->app_url != 'none'  && $rowp->app_type == '2') { //Check if the value is a PDF file ?>
                                                <a href="wp-content/themes/orchid-new/assets/pdf/<?php echo $rowp->app_url ?>" title="<?php echo $rowp->tile_name ?>" target="_blank" class="href-nav">
                                            <?php } else if ($rowp->app_url != 'none'  && $rowp->app_type == '1') { //Check if the value is non PDF file ?>
                                                <a href="<?php echo $_PHP_SELF; ?><?php echo $rowp->app_url ?>"  title="<?php echo $rowp->tile_name ?>" target="_blank" class="href-nav">
                                            <?php } ?>
                                            <img src="wp-content/themes/orchid-new/assets/img/<?php echo $rowp->image ?>" border="0" class="ind-a" title="<?php echo $rowp->tile_name ?>" />
                                            <div class="tiles-t"><?php echo $rowp->tile_name ?></div>
                                        </a>
                                    <?php } ?>
                                </div>
                            </div>
                 <?php } ?>
                        </div>
                     </div>
                 </div>
                 <div class="col-lg-4 col-md-4">
                     <div class="lead text-center" style="color:#0057b8">Commercial Lines<br/>
                           <span style="font-size:0.8em;">Select a product to create a new quote.</span>
                     </div>
                     <div class="container-fluid">
                           <div id="rowC" class="rowC" style="padding-left:30px;padding-top:8px;">
                             <?php foreach ($resultsc as $rowc) { //Start Commercial Lines ?>

                                            <div id="tileC" class="filterrowC" data-state="<?php echo $rowc->states; ?>">
                                                <div class="col-sm-4" style="background-color:#FFFFFF;">
                                                    <?php if ($rowc->occurring == '2') { //Check for coming soon YES=2 ?>
                                                        <span class="sm-4-corner">Coming soon</span>
                                                        <div  name="footprintinfo" class="footprintinfo" style="position:absolute;left:122px;z-index:2147483647;"></div>
                                                        <img src="wp-content/themes/orchid-new/assets/img/<?php echo $rowc->image ?>" border="0" class="ind-a" title="<?php echo $rowc->tile_name ?>" />
                                                        <div class="tiles-t"><?php echo $rowc->tile_name ?></div>
                                                    <?php } else if ($rowc->occurring == '1') { //Check for coming soon NO=1 ?>
                                                        <?php if ($rowc->app_url == 'none' && $rowc->app_type == '3') {  //Check to see if there is a PDF or Link ?>
                                                            <a href="#" onclick="getCLine('<?php echo $rowc->getLine ?>','<?php echo $rowc->tile_name ?>','none');" class="href-nav" title="<?php echo $rowc->tile_name; ?>">
                                                        <?php } else if ($rowc->app_url != 'none' && $rowc->app_type == '2') { //Check if the value is a PDF file ?>
                                                            <a href="wp-content/themes/orchid-new/assets/pdf/<?php echo $rowc->app_url ?>" title="<?php echo $rowc->tile_name ?>" target="_blank" class="href-nav">
                                                        <?php } else if ($rowc->app_url != 'none' && $rowc->app_type == '1') { //Check if the value is non PDF file ?>
                                                            <a href="<?php echo $rowc->app_url ?>"  title="<?php echo $rowc->tile_name ?>" target="_blank" class="href-nav">
                                                        <?php } ?>
                                                            <img src="wp-content/themes/orchid-new/assets/img/<?php echo $rowc->image ?>" border="0" class="ind-a" title="<?php echo $rowc->tile_name ?>" />
                                                            <div class="tiles-t"><?php echo $rowc->tile_name ?></div>
                                                        </a>
                                                    <?php } ?>
                                                </div>
                                            </div>

                             <?php } ?>
                           </div>
                       </div>
                   </div>
                   <div class="col-md-2 col-md-offset-2">
                       <span class="h2-a" id="announcements">Announcements</span>
                       <div class="col-ann">
                      	<?php $query = new WP_Query( array( 'category_name' => 'Announcements','orderby' => 'post_title', 'order' => 'ASC' ) ); ?>
                      	<?php if ( $query->have_posts() ) : while ( $query->have_posts() ) : $query->the_post(); ?>
                              <div class="row-1"><div class="col-lg-1"><h4 class="h4-a"><?php the_title(); ?></h4><div class="annTypeColor" style="color: #006cb4;margin-top:-8px;margin-right:10px;"><i class="material-icons md-18">&#xE85A;</i></div><span style="float:left;padding:0px;margin-top:-3px;"><?php the_content(); ?></span></div></div>
                          <?php endwhile;
                          wp_reset_postdata();
                          else : ?>
                              <span class="h2-a" id="announcements">Announcements</span><div class="col-ann"><div class="row-1"><div class="col-lg-1"><p><?php esc_html_e( 'Sorry, no posts matched your criteria.' ); ?></p></div></div></div>
                          <?php endif; ?>
                       </div>
                   </div>
             <?php endwhile; else: ?>
                 <?php _e( 'Sorry, no pages matched your criteria.', 'textdomain' ); ?>
             <?php endif; ?>
         </div>

    </div>
    <!-- //END Content -->

   </div>
   <!-- //END Container Normal -->
  </section>

 <?php get_footer(); ?>
